...
import { useCompanies, useJobs } from "../store/queries";
...
function buildGroups(jobs: JobLight[]): Group[] {
  ...
  for (const [company, list] of map) {
    ...
    // Sort roles inside a company: tier desc, then odds rank, then title.
    list.sort(
      (a, b) =>
        b.tier - a.tier ||
  ...
  // Accordion order: max tier desc, then open-role count desc, then name.
  groups.sort(
    (a, b) =>
      b.maxTier - a.maxTier ||
...
export default function Companies() {
  ...
  const visible = useMemo(() => {
    ...
    if (!q) return groups;
    return groups.filter((g) => g.company.toLowerCase().includes(q));
  }, [groups, query]);
    ...
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 12, flexWrap: "wrap" }}>
        ...
        <span className="muted">
          {visible.length} companies · {jobs.length} roles
          {isLoading ? " (loading…)" : ""}
        </span>
        <input
          ...
          return (
            ...
            >
              ...
              >
                <span
                  className="muted"
                  style={{ width: 12, flex: "0 0 auto", fontSize: 10 }}
                >
                  {isOpen ? "▼" : "▶"}
                </span>
